...
# ─────────────────────────────────────────────
# BGC CLI Wrapper
# Uses the installed bgc CLI to call Bitget APIs
# ─────────────────────────────────────────────
...
def bgc(module: str, command: str, **kwargs) -> dict | None:
    ...
    # Find bgc — on Windows it's bgc.cmd
    ...
    if bgc_path:
        ...
        # Final fallback: shell=True lets Windows resolve .cmd itself
    ...
    try:
        result = subprocess.run(
            ...
            env=env, timeout=15, shell=use_shell
        )
        if result.returncode != 0:
            print(f"[execution] bgc error: {result.stderr.strip()}")
            return None
        return json.loads(result.stdout)
    except subprocess.TimeoutExpired:
        print(f"[execution] bgc timeout: {module} {command}")
        return None
    except json.JSONDecodeError as e:
        print(f"[execution] JSON parse error: {e}")
        return None
    except Exception as e:
        print(f"[execution] bgc call failed: {e}")
        return None
